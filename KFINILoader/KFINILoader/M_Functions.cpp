	...
	switch (Function)
	{
		...
		*/
		...
		case SystemKF::fGetPerObjectNames:
			...
			//if (InPutAddress_1)
			{
				/*WriteLog(L"KF UCC.log", 1, "InputAddress n1: ", InPutAddress_1);

				WriteLog(L"KF UCC.log", 1, "InputAddress n2: ", InPutAddress_2);

				if (IsInINIList(InPutAddress_1))
				{
					WriteLog(L"KF UCC.log", 0, "", "");
					WriteLog(L"KF UCC.log", 1, InPutAddress_1, " is in list!!!");
					WriteLog(L"KF UCC.log", 0, "", "");
